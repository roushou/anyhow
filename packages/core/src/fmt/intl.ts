...
export const currency = (value: number, currency: string, loc = locale()) =>
  numCache
    .getOrSet(`num:${loc}:${currency}`, () => new Intl.NumberFormat(loc, { style: "currency", currency }))
    .format(value);
...
export const date = (value: Date, opts?: Intl.DateTimeFormatOptions | "full" | "short", loc = locale()) => {
  const resolved = opts === "full" ? { dateStyle: "full" as const } : opts === "short" ? { dateStyle: "short" as const } : opts;
  return dtCache.getOrSet(`dt:${loc}:${JSON.stringify(resolved)}`, () => new Intl.DateTimeFormat(loc, resolved)).format(value);
};

export const relativeTime = (value: number, unit: Intl.RelativeTimeFormatUnit, loc = locale()) =>
  rtCache.getOrSet(`rt:${loc}`, () => new Intl.RelativeTimeFormat(loc, { numeric: "auto" })).format(value, unit);

export const list = (items: string[], type: "conjunction" | "disjunction" = "conjunction", loc = locale()) =>
  liCache.getOrSet(`list:${loc}:${type}`, () => new Intl.ListFormat(loc, { style: "long", type })).format(items);
...
export const relativeTimeFromNow = (date: Date, now = new Date(), loc = locale()): string => {
  ...
  for (const { unit, seconds } of RT_UNITS) {
    if (abs >= seconds || unit === "second") {
      ...
      return rtCache.getOrSet(`rt:${loc}`, () => new Intl.RelativeTimeFormat(loc, { numeric: "auto" })).format(value, unit);
    }
  }
  return "";
};
